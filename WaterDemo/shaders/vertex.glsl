...
void main()
{
    Wave wave = Wave(vec3(1.0, 0.0, -0.3), 1.0, 3.0);

    vec3 pos = aPos;
    vec3 dir = normalize(vec3(1.0, 0.0, -0.5));
    pos.y += wave(pos, wave);
    vec3 tangent = vec3(1.0, dir.x * waveDerivative(pos, wave), 0.0); // tangent on x axis
    vec3 binormal = vec3(0.0, dir.z * waveDerivative(pos, wave), 1.0); // tangent on z axis
